...
> Authors: Ryosuke Satoh, Rodney Van Meter  
...
<!-- Done -->
The first step of connection setup is issuing `Connection Setup Request` for Responder. This request contains a comprehensive set of information regarding application and its performance requirements. Additionally, the request packet includes the routed path information from Initiator to Responder.
...
> Assumption: Resource allocation algorithms must be shared or configured beforehand
> If we change resource allocation method in LAU process, need more number of processes to negotiate it.
### 4.6 Barrier
