...
## Abstract
The very first step for an application to be responsible for is to establish a connection from one end node to another. Establishing connection in quantum network may require more information than that of classical network especially for near-term quantum networking at which is fragile to loss and noise of quantum system. This document specifies how end nodes in quantum network establish connection between them and teardown the established connection after they used it.

## 1. Introduction
Establishing a connection across a quantum network [^KIM2008] is the first fundamental classical task for running a quantum network application. Due to the photon loss and fragility of the quantum information, quantum communication between two (or more) end nodes requires a coordinated computation among the set of quantum repeaters on the path rather than the current store-and-forward network session [^VAN2014].

This connection setup process assumes running quantum networking application on two end nodes. Multiparty application is beyond the scope of this specification.
