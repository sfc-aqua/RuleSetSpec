...
- [Draft: Connection Setup and Teardown](#draft-connection-setup-and-teardown)
  ...
  - [4. Connection Setup](#4-connection-setup)
    ...
    - [4.2 RuleSet and Performance Indicator](#42-ruleset-and-performance-indicator)
    - [4.3 Connection Setup in single network](#43-connection-setup-in-single-network)
    - [4.4 Link Allocation Policy](#44-link-allocation-policy)
    - [4.5 Connection Setup Rejection](#45-connection-setup-rejection)
    - [4.6 Null connection setup request](#46-null-connection-setup-request)
  - [5. Connection Teardown](#5-connection-teardown)
    - [5.1 Connection Teardown Phases](#51-connection-teardown-phases)
    - [5.2 Expected RuleSet Termination](#52-expected-ruleset-termination)
...
Qubit is a unit of information and sometimes refers to physical devices that stores one bit of quantum data.
...
## 4. Connection Setup
<!-- 4.1 Connection Setup Phases: Review overall connection setup process -->
<!-- 4.2 Explain the performance indicators and RuleSet -->
<!-- 4.3 Show an example of single network connection setup -->
<!-- 4.4 How each link allocate resources to the RuleSet -->
<!-- 4.5 Explain the situation that connection is rejected -->
<!-- 4.6 Introduce Null Connection Setup to check the status of the path -->
...
### 4.2 RuleSet and Performance Indicator
*RuleSet* [^MAT2019] is a set of *Match-Action* style instructions that is designed to coordinate intermediate quantum repeaters. RuleSet contains a set of meta information that identify the connection and corresponding application and a set of *Rules* to be performed.

RuleSet is created for each quantum repeater at Responsder based on `Performance Indicator` they provide.

### 4.3 Connection Setup in single network
...
### 4.5 Connection Setup Rejection
...
### 4.6 Null connection setup request
...
### 5.2 Expected RuleSet Termination
