...
In the quantum communication, one of the fundamental netowork resources is Bell state which is bipertite quantum entangled state. Throughout this document the term "resource" referes to Bell pair unless there is annotation on it.
<!-- resource allocation -->
...
> ADD Expalanation of link allocation policy negotiation
...
```json
{
  ...
  "properties": {
    "linkResourceID": {
      "type": "number",
      "description": "Incremental resource id for link Bell pair for switch timing alignment."
    }
  }
}
